...
namespace inst::ui
{
    ...
    void netInstPage::startInstall(bool urlMode)
    {
        ...
        if (this->selectedUrls.size() == 1)
        {
            ...
            dialogResult = mainApp->CreateShowDialog("inst.target.desc00"_lang + std::to_string(this->selectedUrls.size()) + "inst.target.desc01"_lang, "common.cancel_desc"_lang, {"inst.target.opt0"_lang, "inst.target.opt1"_lang}, false);
        ...
        netInstStuff::installTitleNet(this->selectedUrls, this->username, this->password, dialogResult, this->alternativeNames, sourceString);
        return;
    }
